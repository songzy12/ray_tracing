...
class World {	
	public:
		...
		double						eye; // view point z coordinate, for no camera only
		double						d; // distance from view plane, for no camera only*/
	public:
		...
		void
		display_pixel(const int row, const int column, const RGBColor& pixel_color, cv::Mat &img) const;

		ShadeRec
		hit_objects(const Ray &ray);

		/*ShadeRec hit_bare_bones_objects(const Ray &ray); // no const here, for multiple objects only
		
		void 												
		render_scene(cv::Mat &img) const; // reference or not, for no camera only

		void render_perspective(cv::Mat &img) const;// for no camera only*/
